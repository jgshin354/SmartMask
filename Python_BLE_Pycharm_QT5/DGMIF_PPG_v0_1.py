...
class Form(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.pw1 = self.ui.red.addPlot(row=0, col=0, title='RED/IR reflection')
        self.pw2 = self.ui.red.addPlot(row=0, col=1, title='Heartbeat rate')
        self.pw3 = self.ui.red.addPlot(row=1, col=0, title='SpO2')
        self.pw4 = self.ui.red.addPlot(row=1, col=1, title='Temperature')
        #self.pw5 = self.ui.red.addPlot(row=1, col=2, title='Temperature')
        #self.pw1_plot = self.pw1.plot()
        self.pw2_plot = self.pw2.plot()
        self.pw3_plot = self.pw3.plot()
        self.pw4_plot = self.pw4.plot()
        #self.pw5_plot = self.pw5.plot()
    ...
    def file_save_action(self):
        ...
        if fname[0]:
            ...
            for i in range(0,len(self.spo2)):
                print(i)
                text_tmp = str(self.date_hr_spo2[i]) + ',' + str(self.time_hr_spo2[i]) +','+ str(self.hr[i])+',' + str(self.spo2[i]) +',' + str(self.temperature[i]) + '\n'
                # print(text_tmp)
    ...
    def fig_update(self):
        #self.pw1_plot.setData(self.red_plot, pen='r', name='red plot')
        #self.pw1_plot.setData(self.ir_plot, pen='w', name='ir plot')
        #self.pw1_plot.plot(self.red_plot, pen='r', name='red plot')
        #self.pw1_plot.plot(self.ir_plot, pen='w', name='ir plot')
        self.pw1.clear()
        self.pw1.plot(self.red_plot, pen='r', name='red plot')
        self.pw1.plot(self.ir_plot, pen='w', name='ir plot')
        self.pw1.addLegend()

        #self.pw1_plot.setData(self.red_plot, pen='r', name='red plot')
        #self.pw2_plot.setData(self.ir_plot)
        self.pw2_plot.setData(self.hr)
        self.pw3_plot.setData(self.spo2)
        self.pw4_plot.setData(self.temperature)
    ...
    def notify_callback(self, sender: int, data: bytearray):
        if data[0] == 1:
            ...
            if data[4] == 1:
                hr_curr = data[1]
                spo2_curr = data[3]
                temp_curr = int.from_bytes(data[5:8], "little") / 100
                #########################################
                ...
                self.temperature = np.append(self.temperature, temp_curr)
                self.date_hr_spo2.append(date_curr)
                self.time_hr_spo2.append(time_curr)
                text_tmp = date_curr + ',  ' +time_curr + ', heartbeat rate:' + str(hr_curr) + ', SpO2: ' + str(spo2_curr) + '%' + ', temp: '+ str(temp_curr)
