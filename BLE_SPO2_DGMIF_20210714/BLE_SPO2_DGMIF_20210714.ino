...
*/
  ...
  particleSensor.enableDIETEMPRDY(); //[temp] Enable the temp ready interrupt. This is required.
  ...
  if (deviceConnected)  {
    ...
    while (deviceConnected)
    {
      ...
      for (int i = bufferLength - 25; i < bufferLength; i++)
      {
        ...
        memcpy(bluetooth_send + 5, &irBuffer[i], sizeof(uint32_t));
//        bluetooth_send[0] = 1;
...
//        bluetooth_send[4] = 0;
        ...
        pCharacteristic->setValue((uint8_t*)&bluetooth_send, 9); //bluetooth value send
        pCharacteristic->notify();                  //bluetooth value send
        /*
          Serial.print(F("red="));
          Serial.print(redBuffer[i], DEC);
      ...
      uint32_t temperature_uint = uint32_t(temperature*100);
      ...
      memcpy(bluetooth_send + 5, &temperature_uint, sizeof(uint32_t)); //sizeof(float)=4
//      
